(()=>{
  const bust='?clean=0601-20260913';
  Promise.all([
...
function drawWallColumn`);
    ...
    document.title='The Winter Registry — 0.6.1 Clean Village Build';
    const note=document.querySelector('.small-note');
    if(note)note.textContent='BUILD 0.6.1 CLEAN · long sightlines · low perimeter fence · car-blocked main entrance · CACHE-FREE TEST BUILD';
    (0,eval)(src);
  }).catch(err=>{
    console.error(err);
    document.body.innerHTML='<main style="padding:32px;background:#111;color:#ddd;font-family:monospace"><h1>Winter Registry 0.6.1 failed to initialise</h1><pre>'+String(err)+'</pre></main>';
  });
})();
