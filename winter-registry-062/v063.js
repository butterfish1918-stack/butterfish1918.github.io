(()=>{
  ...
  Promise.all([
    ...
    src=src
      ...
      .replace('No compatible 0.3 save found','No compatible 0.6.3 save found')
    ...
    render=render
      ...
      .replace("const fog=clamp((dist-4.5)/10,0,.72),fogC=[143,151,145]","const fog=clamp((dist-22)/40,0,.08),fogC=[143,151,145]")
      .replace("const fog=clamp((corrected-4.8)/8.5,0,.86)","const fog=clamp((corrected-22)/40,0,.08)")
      .replace("if(!p||p.dist>11.5)continue","if(!p||p.dist>28)continue")
      .replace("const H=clamp(110/p.dist,8,64);","const H=clamp((s.type==='car'?170:110)/p.dist,s.type==='car'?12:8,s.type==='car'?88:64);")
...
function drawFenceSection(x1,y1,x2,y2,fov,bob){
  const len=Math.hypot(x2-x1,y2-y1),parts=Math.max(1,Math.ceil(len/.75));
  for(let i=0;i<parts;i++){
    const t1=i/parts,t2=(i+1)/parts;
    const ax=x1+(x2-x1)*t1,ay=y1+(y2-y1)*t1,bx=x1+(x2-x1)*t2,by=y1+(y2-y1)*t2;
    const a0=projectWorldPoint(ax,ay,0,fov,bob),a1=projectWorldPoint(ax,ay,.48,fov,bob);
    const b0=projectWorldPoint(bx,by,0,fov,bob),b1=projectWorldPoint(bx,by,.48,fov,bob);
    if(!a0||!a1||!b0||!b1)continue;
    if((a0.x<-24&&b0.x<-24)||(a0.x>w+24&&b0.x>w+24))continue;
    ctx.strokeStyle='rgba(72,70,61,.78)';
    ctx.lineWidth=Math.max(1,Math.min(2,4/((a0.d+b0.d)*.5)));
    ctx.beginPath();ctx.moveTo(a0.x,a0.y);ctx.lineTo(a1.x,a1.y);ctx.moveTo(b0.x,b0.y);ctx.lineTo(b1.x,b1.y);ctx.stroke();
    for(const z of [.18,.36]){
...
function drawVillageLandmarks(fov,bob,time){
  ...
  }).filter(o=>o.p&&o.p.x>-w*.28&&o.p.x<w*1.28)
    .sort((a,b)=>b.dist-a.dist);
  for(const o of items){
    const {b,p}=o;
    const worldWidth=Math.max(1.3,Math.max(b.x1-b.x0+1,b.y1-b.y0+1)*.72);
    let bh=Math.max(9,p.scale*b.height),bw=Math.max(10,Math.min(w*.42,p.scale*worldWidth));
    const bottom=p.y,top=bottom-bh,left=p.x-bw*.5;
    const c=landmarkColor(b);
    const haze=clamp((o.dist-14)/18,0,.12);
    ctx.save();
    ctx.globalAlpha=.95;
    ctx.fillStyle=`rgb(${Math.round(c[0]*(1-haze)+143*haze)},${Math.round(c[1]*(1-haze)+151*haze)},${Math.round(c[2]*(1-haze)+145*haze)})`;
    ctx.fillRect(Math.round(left),Math.round(top),Math.ceil(bw),Math.ceil(bh));
    ctx.fillStyle='rgba(31,34,31,.58)';
    ...
    if(b.id==='chapel'){
      ...
      ctx.fillRect(Math.round(left-bw*.03),Math.round(top-roofH),Math.ceil(bw*1.06),roofH+1);
    }
    const floors=Math.max(1,b.floors),cols=b.id==='warehouse'||b.id==='workshop'?4:3;
    ctx.fillStyle='rgba(31,42,42,.72)';
    for(let fy=0;fy<floors;fy++){
      const wy=top+bh*(.2+(fy/floors)*.56);
      for(let wx=0;wx<cols;wx++){
        const ww=Math.max(1,bw*.10),wh=Math.max(1,bh*.055);
        const xx=left+bw*(.16+wx*(.68/Math.max(1,cols-1)))-ww*.5;
        ctx.fillRect(Math.round(xx),Math.round(wy),Math.ceil(ww),Math.ceil(wh));
      }
    }
    if(b.id==='clinic'){
      ctx.fillStyle='rgba(133,50,45,.85)';ctx.fillRect(Math.round(p.x-1),Math.round(top+bh*.18),2,Math.max(2,Math.round(bh*.13)));
    }
    ctx.restore();
  }
}
`;
    ...
    render=render.replace(
      "function renderWorld(time){ctx.clearRect(0,0,w,h);ctx.imageSmoothingEnabled=false;const bob=Math.round(Math.sin(state.player.bob)*1.5);drawSkyGround(bob,time);const fov=Math.PI/2.0,cols=w;",
      "function renderWorld(time){ctx.clearRect(0,0,w,h);ctx.imageSmoothingEnabled=false;const bob=Math.round(Math.sin(state.player.bob)*1.5);drawSkyGround(bob,time);const fov=Math.PI/2.0;drawVillageLandmarks(fov,bob,time);drawPerimeterFence(fov,bob);const cols=w;"
    );

    render=render.replace(
      "if(b.id==='edge'){if((x+y)%2===0){mapCtx.fillStyle='#687267';mapCtx.fillRect(x*ts+ts*.35,y*ts+ts*.35,ts*.3,ts*.3)}continue}",
      "if(b.id==='edge')continue"
    );
    render=render.replace(
      "mapCtx.fillStyle='#231f1a';mapCtx.fillRect(Math.round(state.player.x*ts)-2,Math.round(state.player.y*ts)-2,4,4);",
      "mapCtx.strokeStyle='#5d5b50';mapCtx.lineWidth=1;mapCtx.beginPath();mapCtx.moveTo(ts,ts);mapCtx.lineTo(15*ts,ts);mapCtx.lineTo(15*ts,15*ts);mapCtx.moveTo(ts,ts);mapCtx.lineTo(ts,15*ts);mapCtx.moveTo(ts,15*ts);mapCtx.lineTo(7*ts,15*ts);mapCtx.moveTo(9*ts,15*ts);mapCtx.lineTo(15*ts,15*ts);mapCtx.stroke();mapCtx.fillStyle='#343833';mapCtx.fillRect(7.0*ts,13.95*ts,1.0*ts,.32*ts);mapCtx.fillStyle='#231f1a';mapCtx.fillRect(Math.round(state.player.x*ts)-2,Math.round(state.player.y*ts)-2,4,4);"
    );
    ...
    document.title='The Winter Registry — 0.6.3 Persistent Village Build';
    const note=document.querySelector('.small-note');
    if(note)note.textContent='BUILD 0.6.3 · persistent distant buildings · stable low perimeter fence · car-blocked gate · no pop-in';
    const badge=document.createElement('div');
    badge.textContent='0.6.3 · PERSISTENT VILLAGE BUILD';
    ...
    console.error(err);
    document.body.innerHTML='<main style="padding:32px;background:#111;color:#ddd;font-family:monospace"><h1>Winter Registry 0.6.3 failed to initialise</h1><pre>'+String(err)+'</pre></main>';
  });
})();
